//
// Modified by shabaz on 10/02/2023.
//
...
SPIFBlockDevice::SPIFBlockDevice(
        uint8_t sport, PinName mosi, PinName miso, PinName sclk, PinName csel, int freq)
        : m_spi_port(sport), m_spi_pin_mosi(mosi), m_spi_pin_miso(miso), m_spi_pin_sclk(sclk), m_spi_pin_cs(csel), _device_size_bytes(0), _is_initialized(false), _init_ref_count(0)
{
    ...
    spi_init(SPORT, freq);
    gpio_set_function(m_spi_pin_mosi, GPIO_FUNC_SPI);
    gpio_set_function(m_spi_pin_miso, GPIO_FUNC_SPI);
    gpio_set_function(m_spi_pin_sclk, GPIO_FUNC_SPI);
    gpio_init(m_spi_pin_cs);
    gpio_set_dir(m_spi_pin_cs, GPIO_OUT);
...
int SPIFBlockDevice::init()
{
    ...
    if ( (sector_map_table_addr != 0) && (0 != sector_map_table_size) ) {
        tr_info("INFO: init - Parsing Sector Map Table - addr: 0x%lxh, Size: %d\n", sector_map_table_addr,
...
int SPIFBlockDevice::program(const void *buffer, bd_addr_t addr, bd_size_t size)
{
    ...
    tr_debug("DEBUG: program - Buff: 0x%lxh, addr: %llu, size: %llu\n", (uint32_t)buffer, addr, size);
...
int SPIFBlockDevice::erase(bd_addr_t addr, bd_size_t in_size)
{
    ...
    tr_info("DEBUG: erase - addr: %llu, in_size: %llu\n", addr, in_size);
...
spif_bd_error SPIFBlockDevice::_spi_send_read_command(int read_inst, uint8_t *buffer, bd_addr_t addr, bd_size_t size)
{
    ...
    gpio_put(m_spi_pin_cs, 0);
    //_cs = 0;

    // Write 1 byte Instruction
    //_spi.write(read_inst);
    tbuf[0]=(uint8_t)read_inst;
    spi_write_blocking(SPORT, tbuf, 1);
    ...
    for (int address_shift = ((_address_size - 1) * 8); address_shift >= 0; address_shift -= 8) {
        ...
        tbuf[0]=(uint8_t)((addr >> address_shift) & 0xFF);
        spi_write_blocking(SPORT, tbuf, 1);
    }
    ...
    for (bd_size_t i = 0; i < size; i++) {
        ...
        spi_read_blocking(SPORT, 0, tbuf, 1);
        buffer[i] = tbuf[0];
    }
                                                         ...
                                                         bd_size_t size)
{
   ...
   tbuf[0]=(uint8_t)prog_inst;
    ...
    for (bd_size_t i = 0; i < size; i++) {
        //_spi.write(data[i]);
        spi_write_blocking(SPORT, &data[i], 1);
    }
...
{
    tr_info("INFO: Erase Inst: 0x%xh, addr: %llu, size: %llu\n", erase_inst, addr, size);
                                                         ...
                                                         size_t tx_length, char *rx_buffer, size_t rx_length)
{
    ...
    if (addr != SPI_NO_ADDRESS_COMMAND) {
        ...
        for (uint32_t i = 0; i < dummy_bytes; i++) {
            //_spi.write(dummy_byte);
            spi_write_blocking(SPORT, &dummy_byte, 1);
        }
    }
                                              ...
                                              uint32_t& sector_map_table_addr, size_t& sector_map_table_size)
{
    ...
    tr_debug("DEBUG: number of Param Headers: %d\n", number_of_param_headers);
    ...
    for (int i_ind = 0; i_ind < number_of_param_headers; i_ind++) {
        ...
        if (status != SPIF_BD_ERROR_OK) {
            tr_error("ERROR: init - Read Param Table %d Failed\n", i_ind + 1);
            return -1;
        }
        ...
        if (param_header[2] != 1) {
            tr_error("ERROR: Param Table %d - Major Version should be 1!\n", i_ind + 1);
            return -1;
        }

        if ((param_header[0] == 0) && (param_header[7] == 0xFF)) {
            // Found Basic Params Table: LSB=0x00, MSB=0xFF
            tr_debug("DEBUG: Found Basic Param Table at Table: %d\n", i_ind + 1);
            ...
            tr_debug("DEBUG: Found Sector Map Table at Table: %d\n", i_ind + 1);
...
unsigned int SPIFBlockDevice::_sfdp_detect_page_size(uint8_t *basic_param_table_ptr, int basic_param_table_size)
{
    ...
    if (basic_param_table_size > SPIF_BASIC_PARAM_TABLE_PAGE_SIZE_BYTE) {
        ...
        tr_debug("DEBUG: Detected Page Size: %d\n", page_size);
    } else {
        tr_debug("DEBUG: Using Default Page Size: %d\n", page_size);
    }
    return page_size;
}
                                                            ...
                                                            int *erase_type_inst_arr, unsigned int *erase_type_size_arr)
{
    ...
    if (basic_param_table_size > SPIF_BASIC_PARAM_ERASE_TYPE_1_SIZE_BYTE) {
        ...
        for (int i_ind = 0; i_ind < 4; i_ind++) {
            ...
            tr_info("DEBUG: Erase Type(A) %d - Inst: 0x%xh, Size: %d\n", (i_ind + 1), erase_type_inst_arr[i_ind],
            ...
            tr_info("INFO: Erase Type %d - Inst: 0x%xh, Size: %d\n", (i_ind + 1), erase_type_inst_arr[i_ind],
                    erase_type_size_arr[i_ind]);
            bitfield = bitfield << 1;
        }
    }
                                                     ...
                                                     int& read_inst)
{
    do {
        ...
        tr_debug("\nDEBUG: Read Bus Mode set to 1-1-1, Instruction: 0x%xh\n", read_inst);
    } while (false);
